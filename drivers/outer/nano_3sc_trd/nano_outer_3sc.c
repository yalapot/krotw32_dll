...
/*
unsigned __stdcall Thread_01_Func( void* pArguments )
{
    (void) pArguments;

    MessageBox(NULL, "Thread_01_Func message !!!", "Thread_01_Func",MB_OK | MB_SYSTEMMODAL);
...
HANDLE hThread_2;
unsigned thread_2_ID;

unsigned __stdcall Thread_02_Func( void* pArguments )
{
    (void) pArguments;

    MessageBox(NULL, "Thread_02_Func message !!!", "Thread_02_Func",MB_OK | MB_SYSTEMMODAL);
                           ...
                           long length, KRTDATA *screen, long *rowData)
{
  ...
  T_GARY_ARC_THREAD_ARG Gary_arc_thread_arg[Num_parallel_thread];
  long num_Thread_for_use;
    ...
    if ( (last_start != start) || (last_length != length) || (pred_trace_length != trace_list[cur_trace].trace->trace_len))
    {
        ...
        free_files_data( &(trace_list[cur_trace].trace->Files_buf) );
        ...
        for  ( last_spin_num = first_spin_num;
               last_spin_num < trace_list[cur_trace].trace->idx_head.num_idx_in_table-2;
               last_spin_num++
             )
        {
        ...
        if ((spin_num > 0) )
        {
            load_files_data( &(trace_list[cur_trace].trace->Files_buf), 
                             trace_list[cur_trace].trace->idx_trc[first_spin_num-1].file_num,
                             trace_list[cur_trace].trace->idx_trc[last_spin_num+1].file_num,
                             trace_list[cur_trace].trace->path_data);

            if (Gary_arc(trace_list[cur_trace].trace, spin_num-1, &spin[0]) != KRT_OK)
            {
               strcat(drvError, "(krtDrvFillData)");
               return KRT_ERR;
            };

            Smooth_spin(trace_list[cur_trace].trace, &spin[0]);
 
            if (spin[0].item != NULL) spin[0].item = free_inquiry(spin[0].item);

        } else {
            load_files_data( &(trace_list[cur_trace].trace->Files_buf), 
                             trace_list[cur_trace].trace->idx_trc[first_spin_num].file_num,
                             trace_list[cur_trace].trace->idx_trc[last_spin_num+1].file_num,
                             trace_list[cur_trace].trace->path_data);
        };
        // если последний оборот не был предыдущим вызвали предыдущий оборот, чтоб математика настроилась

        for (spin_num = 0; spin_num < num_spin_need; spin_num += Num_parallel_thread)
        {
            num_Thread_for_use = num_spin_need - spin_num;
            if (num_Thread_for_use > Num_parallel_thread) num_Thread_for_use = Num_parallel_thread;

            for (i = 0; i < num_Thread_for_use; i++)
            {
                spin[spin_num + i].item = NULL;

                Gary_arc_thread_arg[i].P_trace   = trace_list[cur_trace].trace;
                Gary_arc_thread_arg[i].spin_num  = spin_num + first_spin_num + i;
                Gary_arc_thread_arg[i].spin_info = &(spin[spin_num + i]);

                hThread_1[i] = (HANDLE)_beginthreadex( NULL, 0, &Thread_Gary_arc, &(Gary_arc_thread_arg[i]), 0, &(thread_1_ID[i]) );
             };
...
/*
        for (segment_counter = 0; segment_counter <= segments_num; segment_counter += Num_parallel_thread)
        {
            Thread_counter = segment_counter;

            num_Thread_for_use = segments_num - segment_counter;
            if (num_Thread_for_use > Num_parallel_thread) num_Thread_for_use = Num_parallel_thread;

            for (i = 0; i < num_Thread_for_use; i++)
            {
                // создаем экземпляр делегата чтоб запустить его в парраллельном потоке
                Copy_segment2Sens[Thread_counter] = new Thread_service_profil().Copy_segment_to_Sens;
                // Стартуем загрузку:
                newThread[Thread_counter] =
                      Copy_segment2Sens[Thread_counter].BeginInvoke(
                           first_dimension_in_segment,
                           (List<DeviceElement>)trace.DeviceSenses,
                           (Profil_segment_trace)trace.Work_Info.idx_data[trace.Work_Info.idx_head.first_load_data + segment_counter+i].Segment,
                           (Int32)(first_dimension_in_sens), // начало от запрошенной дистанции
                           null, null
                      );

                first_dimension_in_sens += 
                      trace.Work_Info.idx_data[trace.Work_Info.idx_head.first_load_data + segment_counter+i].Segment.length
                      - first_dimension_in_segment;
                first_dimension_in_segment = 0;
                Thread_counter++;
            };
            ...
            for (i = 0; i < num_Thread_for_use; i++)
            {
                Copy_segment2Sens[Thread_counter].EndInvoke(newThread[Thread_counter]);
                Thread_counter++;
            }
*/
        ...
        for (spin_num = 0; spin_num < num_spin_need; spin_num++)
        {
            Smooth_spin(trace_list[cur_trace].trace, &(spin[spin_num]) );
        };
    };

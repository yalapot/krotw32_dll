                     ...
                     )
{
   ...
   while (token != NULL) {
      if (!row_header)
      {
         ...
         {
             token_float= token;
             while (token_float[0] != '\0')
             {
                 if (token_float[0]==FloatDelimiter)
                 {
                    token_float[0] = '.';
                 };
                 token_float++;
         ...
         row_buf[column] = (long)(atof(token)*Amlification);
         column ++;
                    ...
                    )
{
   ...
   while (row_str != NULL) {
      ...
      if (!column_header) {
         ...
         pars_csv_string( row_header, row_str, &(table[row_pos_in_table]),
                          column_num, Amlification, FloatDelimiter);
                    ...
                    )
{
   ...
   row_str = strtok(cur_table_str, seps );
   while (row_str != NULL && strlen(row_str) > 2)
   {
       if (!column_header)
       {
          row++;

...
//    };
   ...
   // меняем местами ласты в 1-м секторе       // и 3-го секторов
   for (i=256; i<512; i+=16) {
      for (j=0; j<8; j++) {
         // 1-го
         tmp_sens_sort[0] = opened_trace->sens_sort[ i + j ];

         opened_trace->sens_sort[ i + j ] = 
              opened_trace->sens_sort[ i + j + 8 ];

         opened_trace->sens_sort[ i + j + 8 ] = tmp_sens_sort[0];

         // 3-го
//         tmp_sens_sort[0] = opened_trace->sens_sort[ (i*8) + j + 384];

//         opened_trace->sens_sort[ (i*8) + j + 384] = 
//              opened_trace->sens_sort[ (i*8) + (7 - j) + 384];

//         opened_trace->sens_sort[ (i*8) + (7 - j) + 384] = tmp_sens_sort[0];

       };
    };

/*
   // разворачиваем датчики в ластах 1-го и 3-го секторов
   for (i=0; i<32; i++) {
      for (j=0; j<4; j++) {
         // 1-го
         tmp_sens_sort[0] = opened_trace->sens_sort[ (i*8) + j + 128];

         opened_trace->sens_sort[ (i*8) + j + 128] = 
              opened_trace->sens_sort[ (i*8) + (7 - j) + 128];

         opened_trace->sens_sort[ (i*8) + (7 - j) + 128] = tmp_sens_sort[0];
...
long set_sens_sort_coross_325(T_OPENED_TRACE * opened_trace) {
...
long set_sens_sort_strs_325(T_OPENED_TRACE * opened_trace) {
...
long set_sens_sort_strs_1000(T_OPENED_TRACE * opened_trace)
{
   int i;
   int k;
...
//         opened_trace->sens_sort[ 80 + k*8 + i ] =  80 + k*8 + 7-i;
//
//         opened_trace->sens_sort[240 + k*8 + i ] = 240 + k*8 + 7-i;

         opened_trace->sens_sort[400 + k*8 + i ] = 400 + k*8 + 7-i;
   ...
   for (k=0; k<10; k++)
   {
      for (i=0; i<8; i++) {
         ...
         if (i<4)
         {
             opened_trace->sens_shift[  0 + k*8 + i ] =
             opened_trace->sens_shift[ 80 + k*8 + i ] =
             opened_trace->sens_shift[160 + k*8 + i ] =
             opened_trace->sens_shift[240 + k*8 + i ] =
             opened_trace->sens_shift[320 + k*8 + i ] =
             opened_trace->sens_shift[400 + k*8 + i ] = (long) ( ( 10.5 * (float) k) );
         } else {
             opened_trace->sens_shift[  0 + k*8 + i ] =
             opened_trace->sens_shift[ 80 + k*8 + i ] =
             opened_trace->sens_shift[160 + k*8 + i ] =
             opened_trace->sens_shift[240 + k*8 + i ] =
             opened_trace->sens_shift[320 + k*8 + i ] =
             opened_trace->sens_shift[400 + k*8 + i ] = (long) ( (2 + 10.5 * (float) k) );
         };
...
}; // set_sens_sort_strs_1000(opened_trace)
   ...
   switch (opened_trace->corosion_sens_num) 
    {
     ...
     case (288) : {  // вроде как новая пятисотка (500 корозионник)
        ...
        opened_trace->Orientation_shift=33;
        opened_trace->Orientation_shift_2=0;
...
//        opened_trace->real_number_sensor_in_group[0]=96;
     ...
     case (480) : {  // вроде как 1000 стресс
        if (strcmp(target_name, TARGET_NAME_STRESS1000)==0) {
            ...
            opened_trace->Orientation_OFF = 0;
            opened_trace->Orinteation_Direct_count=0;
            opened_trace->Orientation_shift = -56;
            opened_trace->Orientation_shift_2=0;
            ...
            if (GetPrivateProfileString(EPRO_SECTION, Invert_sens_data_KEY, "нет", key_value,
             ...
             {
                if (strcmp( key_value, "нет") == 0) {
                   strcpy( opened_trace->invert_sens_data_text, "0-479");
...
//            strcmp(target_name, TARGET_NAME_CORROS508_2)==0 ||
           ...
           )
        {
           ...
           opened_trace->sens_for_thick=70;
...
*/
     ...
     case (512) : {  // вроде как СТРЕСС 700 + СИТ

        if (strcmp(target_name, TARGET_NAME_STRS700sit)==0) {
           // Значится СТРЕСС 700 + СИТ
           opened_trace->trace_step = 5;     // шаг опроса 5 мм
           opened_trace->group_num  = 1;      // один пояс
           ...
           if (GetPrivateProfileString(EPRO_SECTION, Invert_sens_data_KEY, "нет", key_value,
            ...
            {
               if (strcmp( key_value, "нет") == 0) {
                  strcpy( opened_trace->invert_sens_data_text, "0-511");
           ...
           if (file_counter==0) {
                ...
                i = MessageBox(NULL, "Диаметр обследованного трубопровода? \n 700 = Да; 800 = Нет     ", "700//800 reg",MB_YESNO | MB_ICONQUESTION | MB_SYSTEMMODAL);
                if (i==6) {
                    ...
                    // Выставили удаление для нужных датчиков
                    opened_trace->real_number_sensor_in_group[0] = 448;
                    opened_trace->Orientation_shift = -168;
                    opened_trace->Orientation_shift_2 = 0;
                } // if (i==6) 
                else
                {
                    ...
                    opened_trace->Orientation_shift = 319;
                    opened_trace->Orientation_shift_2 = 0;
                }
            }; // if (file_counter==0) {
           ...
           set_sens_sort_stress_700(opened_trace);
     ...
     case (528) : {  // вроде как Корозионник 508 (Иран)
        ...
        if (strcmp(target_name, TARGET_NAME_CORROS508)==0) {
           ...
           opened_trace->Orientation_OFF = 0;
           opened_trace->Orinteation_Direct_count=0;
           opened_trace->Orientation_shift=135;
...
//           set_sens_sort_prof_426(opened_trace);
     ...
     case (32) : {  // вроде как профиль 426 или Самописец
        ...
        if (strcmp(target_name, TARGET_NAME_PROFIL325)==0) {
           ...
           opened_trace->Orientation_OFF = 0;
           opened_trace->Orinteation_Direct_count=0;
           opened_trace->Orientation_shift=6;
...
//                };
     ...
     case (64) : {  // вроде как Самописец
        ...
        opened_trace->group_num  = 1;      // два пояса

        opened_trace->real_number_sensor_in_group[0]=
        opened_trace->linesense_size[0] = 64;
...
//             set_sens_sort_coross_325(opened_trace);
              ...
              opened_trace->Orientation_shift=25;
           };

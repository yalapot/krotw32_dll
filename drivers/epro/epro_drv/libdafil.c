   ...
   if ((ver_file == VER_FILE_EPRO_2)  || 
       ...
       (ver_file == VER_FILE_EPRO_32)) 
    {
       ...
       if (ver_file != VER_FILE_EPRO_32) {
              ...
              unarc_data->inquiry[i].power_flags = (unarc_data->inquiry[i].power_flags) >> 8; 
        };
...
/*
               ...
               if (debug_counter==6) {
                 sprintf(tmp_log_str, "%d, %d, %d\n",
                     unarc_data->inquiry[i].accel_x, unarc_data->inquiry[i].accel_y, unarc_data->inquiry[i].Pic_ful_track);
...
*/
              //                                          dAz         KAz
              sA=((double)(unarc_data->inquiry[i].accel_y-594.214)) / (262.704);
              //                                          dAy         KAy
              cA=((double)(unarc_data->inquiry[i].accel_x-661.783)) / (269.6);
           ...
           if (corros_num == 512) { // stress 700
              //                                          dAz         KAz
              cA=((double)(unarc_data->inquiry[i].accel_y-374.875)) / (-272.27);
              //                                          dAy         KAy
              sA=((double)(unarc_data->inquiry[i].accel_x-531.415)) / (263.905);
              unarc_data->inquiry[i].orient =
                   calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
            };
           ...
           if (corros_num == 480) { // stress 1000
              //                                          dAz         KAz
              sA=((double)(unarc_data->inquiry[i].accel_y-358.718)) / (-264.67);
              //                                          dAy         KAy
              cA=((double)(unarc_data->inquiry[i].accel_x-594.021)) / (276.044);
              unarc_data->inquiry[i].orient =
                   calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
            };
           ...
           if (corros_num == 80) { // profile 700
              ...
              unarc_data->inquiry[i].accel_x = unarc_data->inquiry[i].accel_x & 0xFFF;
              unarc_data->inquiry[i].accel_y = unarc_data->inquiry[i].accel_y & 0xFFF;
...
//                   sA=((double)(unarc_data->inquiry[i].accel_x-597.397)) / (273.174);
           ...
           if (corros_num == 192) { // Корозионник 325
              if (strcmp(target_name, TARGET_NAME_CORROS325)==0) {
                   // значит Профиль 1400
                   //                                          dAz         KAz
                   cA=((double)(unarc_data->inquiry[i].accel_y-410.191)) / (265.037);
                   //                                          dAy         KAy
                   sA=((double)(unarc_data->inquiry[i].accel_x-479.247)) / (-274.941);
                   unarc_data->inquiry[i].orient =
                        calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
               } else {
                  // значит
           ...
           if (corros_num == 64) { //Профиль 325
              if (strcmp(target_name, TARGET_NAME_PROFIL325)==0) {
                   // значит Профиль 325
                   //                                          dAz         KAz
                   cA=((double)(unarc_data->inquiry[i].accel_y-526.997)) / (266.381);
                   //                                          dAy         KAy
                   sA=((double)(unarc_data->inquiry[i].accel_x-646.272)) / (266.027);
                   unarc_data->inquiry[i].orient =
                        calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
               } else {
                  // значит ...
           ...
           if (corros_num == 384) { //Корозионник 426
                 ...
                 )
              {
                   ...
                   //                                          dAz         KAz
                   sA=((double)(unarc_data->inquiry[i].accel_y-526.997)) / (266.381);
                   //                                          dAy         KAy
                   cA=((double)(unarc_data->inquiry[i].accel_x-646.272)) / (265.032);
                   unarc_data->inquiry[i].orient =
                        calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
              } else {
                 if (strcmp(target_name, TARGET_NAME_CORROS508)==0 )
                 {
                       // значит Корозионник 508
                       //                                          dAz         KAz
                       sA=((double)(unarc_data->inquiry[i].accel_y-627.279)) / (265.009);
                       //                                          dAy         KAy
                       cA=((double)(unarc_data->inquiry[i].accel_x-533.982)) / (-268.0476);
                       unarc_data->inquiry[i].orient =
                            calkulate_orientation(cA, sA, opened_trace->real_number_sensor_in_group[0]);
                 } else {
                      // значит ...
...
//       else
       ...
       d_time /= opened_trace->frequency_processor;
           ...
           sprintf(tmp_log_str, "%ld; %ld; ", full_trace_counter, d_time ); //, debug_counter);
     ...
     for ( group_counter=0;
         ...
         ) 
      {
         ...
         if ( ((opened_trace->corosion_sens_num==80 ) ||
          ...
          {
            if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL700_2)==0)
             {
               ...
               for (sens_counter=first_sens_in_cur_group; 
                    ...
                    sens_counter++) 
                {
                   ...
                   if (prof_calibr_index <= 1) {
                        p_seam->inquiry[dimension_counter].math_sens[sens_counter] = 199;
                   } else {
                       translate_res = 10*(prof_calibr_index)
                       + 10 *
                       (
                       ...
                       translate_res = 200 - translate_res;

                       p_seam->inquiry[dimension_counter].math_sens[sens_counter] =
                                 (EPRO_CORROSION_SENS_TYPE)  (translate_res+20);
                   };
                   prof_sens_num++;
                }; // for sens_counter=first_sens_in_cur_group; 
               ...
               if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL426)==0)
                {
                   ...
                   if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL700_2)==0 )
                    {
                       // если это первый пояс профиля 700_2
                        prof_sens_num=0;
                        for (sens_counter=first_sens_in_cur_group; 
                             sens_counter < (first_sens_in_cur_group+opened_trace->linesense_size[group_counter]);
                             sens_counter++) 
                         {
                            ...
                            if (prof_calibr_index<=1) {
                                ...
                                p_seam->inquiry[dimension_counter].math_sens[sens_counter] =
                                      (EPRO_CORROSION_SENS_TYPE)  (translate_res + 0);

                            }; // else -> if (prof_calibr_index<=1) {


//                            abs_dim[sens_counter] = 0;
//                            f_shift = 0; 

//                            if (p_seam->inquiry[dimension_counter].math_sens[sens_counter] > 60)
//                            {
//                                abs_dim[sens_counter] = (p_seam->inquiry[dimension_counter].math_sens[sens_counter] - 60); // / 10;
//                                f_shift = sqrt(32400 - (134-abs_dim[sens_counter]) * (134-abs_dim[sens_counter]) ) - 120;
//                                abs_dim[sens_counter] = (long) (f_shift / 5);
//                            };

//                            pred_dim[sens_counter] --;
//                            if (pred_dim[sens_counter] < 0) pred_dim[sens_counter]=0;

//                            if (abs_dim[sens_counter] > pred_dim[sens_counter]) pred_dim[sens_counter] = abs_dim[sens_counter];

//                            if ( (dimension_counter >= pred_dim[sens_counter]) && (pred_dim[sens_counter] > 0) )
//                            {
//                                for (i = pred_dim[sens_counter]; i>=0; i--)
//                                p_seam->inquiry[dimension_counter - i].math_sens[sens_counter] =
//                                          p_seam->inquiry[dimension_counter].math_sens[sens_counter];
//                            };
                    ...
                    } else { // if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL700_2)==0 )

                       if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL500_2)==0 )
                        {
                           // если это первый пояс профиля 500_2
                            prof_sens_num=0;
                            for (sens_counter=first_sens_in_cur_group; 
                                 sens_counter < (first_sens_in_cur_group+opened_trace->linesense_size[group_counter]);
                                 sens_counter++) 
                             {
                                ...
                                if (prof_calibr_index<=1) {
                                    p_seam->inquiry[dimension_counter].math_sens[sens_counter] = 150;
                                 } else {
                                    translate_res = 10*(prof_calibr_index)
                                    + 10 *
                                    (
                                    ...
                                    if ((150-translate_res) <0 ) {
                                        p_seam->inquiry[dimension_counter].math_sens[sens_counter] = 0;
                                     } else {
                                        p_seam->inquiry[dimension_counter].math_sens[sens_counter] =
                                              (EPRO_CORROSION_SENS_TYPE)  (150-translate_res);
                                     };

                                 };
                                prof_sens_num++;
                             }; // for sens_counter=first_sens_in_cur_group; 
                        } else { // if (strcmp(opened_trace->target_name, TARGET_NAME_PROFIL500_2)==0 )

                            // если это первый пояс профиля 325
                   prof_sens_num=0;
                   for (sens_counter=first_sens_in_cur_group; 
                        sens_counter < (first_sens_in_cur_group+opened_trace->linesense_size[group_counter]);
                        sens_counter++) 
...
*/
                 translate_res = p_seam->inquiry[dimension_counter].sens[sens_counter];
/*
                 if ( (p_seam->beg_track > 5254450*2 && p_seam->beg_track < 5254550*2) ||
                      (p_seam->beg_track > 5254850*2 && p_seam->beg_track < 5254900*2) ||
                      (p_seam->beg_track > 5255720*2 && p_seam->beg_track < 5255782*2) ||
                      (p_seam->beg_track > 5258200*2 && p_seam->beg_track < 5258350*2) ||
                      (p_seam->beg_track > 5259750*2 && p_seam->beg_track < 5259800*2) ||
                      (p_seam->beg_track > 5260070*2 && p_seam->beg_track < 5260115*2) ||
                      (p_seam->beg_track > 5262870*2 && p_seam->beg_track < 5262970*2) ||
                      (p_seam->beg_track > 5264150*2 && p_seam->beg_track < 5264250*2) ||
                      (p_seam->beg_track > 5264360*2 && p_seam->beg_track < 5264415*2) ||
                      (p_seam->beg_track > 5264480*2 && p_seam->beg_track < 5264530*2) ||
                      (p_seam->beg_track > 5264750*2 && p_seam->beg_track < 5264800*2) ||
                      (p_seam->beg_track > 5266350*2 && p_seam->beg_track < 5266380*2) ||
                      (p_seam->beg_track > 5266450*2 && p_seam->beg_track < 5266480*2) ||
                      (p_seam->beg_track > 5266500*2 && p_seam->beg_track < 5266530*2)


                    )
                 {
                     if ( (av_sens_old[sens_counter][av_sens_len[group_counter]] / 256 
                            - p_seam->inquiry[dimension_counter].sens[sens_counter] ) > 20
                          ||
        ...
        ) ==  sizeof(T_DESCRIPTOR))
   {
      ...
      if (cur_descriptor.ver_file==VER_FILE_EPRO    ||
         ...
         switch (cur_descriptor.descriptor_type) {
            ...
            default : {
               sprintf (drvError, "Неизвестный дескриптор в файле %s(enumerate_seam_in_files)", index_file->file_name);
               fclose(index_file->file_data);
               index_file->file_data =NULL;
               return KRT_ERR;
             }; // default :
         ...
         sprintf (drvError, "Неизвестная версия файла %s (enumerate_seam_in_files)", index_file->file_name);
         fclose(index_file->file_data);
         index_file->file_data =NULL;
         return KRT_ERR;
       }; // if (cur_descriptor.ver_file==VER_FILE_EPRO)
...
long calkulate_index_files_and_sems(T_TRACE_LIST * p_trace_list, long file_counter)
{
  ...
  static T_RECORD_EXTEND_SENS local_extend_sens;

  memset( &NavData, sizeof(NavData), 0);
  ...
  {
       file_header.proc_frequency = 498078981;
  };
...
//                   index_file->index_seam[seam_counter].head_of_condition_seam->beg_odom;
           ...
           if (unarc_lzw(arc_data, 
                ...
                == KRT_ERR) 
            {
              sprintf(drvError, "file = %d; pos_in_file=%d", file_counter, pos_in_file);
              strcat (drvError, "(calkulate_index_files_and_sems)");
              arc_data = (WORD *) heap_free(arc_data);
              ...
              for (dimension_counter=0;
                   ...
                   dimension_counter++) 
               {
                  ...
                  index_file->index_seam[seam_counter].inquiry[dimension_counter].orient
                      =(WORD) get_orient_from_NAV( my_start,
                                           &NavData,
                                           opened_trace->real_number_sensor_in_group[0]);
...
//               pred_timer_ms=timer_ms;
     ...
     //          if (opened_trace->index_extend_sens.extend_sens.time != 0)
     //          {
                    memcpy( &local_extend_sens, &opened_trace->index_extend_sens.extend_sens, sizeof(T_RECORD_EXTEND_SENS));
     //          }

               if (fwrite(&local_extend_sens,
...
//   for (i=0; i<1000; i++)
//   {
//      fwrite(&local_extend_sens, 1, sizeof(T_RECORD_EXTEND_SENS), ext_data);
//   };

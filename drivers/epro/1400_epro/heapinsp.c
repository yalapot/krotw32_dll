...
__inline void * WINAPI heap_alloc  (long size) {
   return malloc(size);
};

#else

void * WINAPI heap_alloc  (long size) {
...
__inline void * WINAPI heap_free ( void * free_adress) {
  free(free_adress);
  return NULL;
};
...
void * WINAPI heap_free ( void * free_adress) {

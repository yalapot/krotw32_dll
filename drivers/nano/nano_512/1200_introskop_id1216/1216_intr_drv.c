...
void create_sens_shift ( long *sens_shift)
{
   ...
   {
       sens_shift[i+  8] += 14; //9;   смена колеса с 5 мм на 3 мм
       sens_shift[i+  9] += 14; //9;
       sens_shift[i+ 10] += 14; //9;
       sens_shift[i+ 11] += 14; //9;
       sens_shift[i+ 12] += 14; //9;
       sens_shift[i+ 13] += 14; //9;
       sens_shift[i+ 14] += 14; //9;
       sens_shift[i+ 15] += 14; //9;
   }
...
long check_file_ID(char* target_name)
{
      ...
      // интроскоп 1200
      if (strncmp(target_name, "12160101", 8)==0)
      {

          Orientation_dAy_value =  541.926;
          Orientation_KAy_value = -243.133;
          Orientation_dAz_value =  485.997;
          Orientation_KAz_value =  243.994;

          Orientation_shift_direct = 1;

          Orientation_shift_group_1 = -310;
          Orientation_shift_group_2 = 0;

          Odometer_0_sens_value = 522;
          Odometer_1_sens_value = 140;

          return KRT_OK;

      } 

     MessageBox(NULL, "Выберете другой драйвер! \nЭто драйвер интроскоп 1216 (Nano512)","драйвер интроскоп 1216 (Nano512)", MB_OK | MB_ICONQUESTION | MB_SYSTEMMODAL);
     return KRT_ERR;
}

...
#define Invert_sens_string  "0-895"
...
void create_sens_sort ( long *sens_sort)
{
    ...
    for (i = FIRST_SENSLINE_SIZE + 320;
         i < FIRST_SENSLINE_SIZE + 416; i++)
    {
         sens_sort[i+32] = i;
    }

    for (i = FIRST_SENSLINE_SIZE + 416;
         i < FIRST_SENSLINE_SIZE + 448; i++)
    {
         sens_sort[i-96] = i;
    }
...
void create_sens_shift ( long *sens_shift)
{
    ...
    for (i=0; i < FIRST_SENSLINE_SIZE; i+=8)
    {
        sens_shift[i + 0] += 9;
        sens_shift[i + 1] += 9;
        sens_shift[i + 2] += 9;
...
long check_file_ID(char* target_name)
{
      ...
      if  ( strncmp(target_name, "30000101", 8)==0 )
      {
          Orientation_dAy_value =   521.716;
          Orientation_KAy_value =   176.670;
          Orientation_dAz_value =   537.686;
          Orientation_KAz_value =  -176.007;

          Orientation_shift_direct = 0;

          Orientation_shift_group_1 = -137;
          Orientation_shift_group_2 = -114;

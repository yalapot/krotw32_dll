...
void create_sens_shift ( long *sens_shift)
{
   ...
   for (i=0; i < MAGN_SENSORS/8; i++) {
      sens_shift[i + MAGN_SENSORS/8 * 0 ] = 635;
      sens_shift[i + MAGN_SENSORS/8 * 2 ] = 635;
      sens_shift[i + MAGN_SENSORS/8 * 4 ] = 635;
      sens_shift[i + MAGN_SENSORS/8 * 6 ] = 635;
   };
...
long check_file_ID(char* target_name)
{
      ...
      if (strncmp(target_name, "12120102", 8)==0)
      {

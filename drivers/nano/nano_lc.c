              ...
              )
{
   ...
   for (sens_counter = 0; sens_counter < num_sens; sens_counter++) {
       ...
       while (repeat_counter < num_test_in_block) {
          ...
          if ( arc_table[table_counter].num_bit < 4) arc_table[table_counter].num_bit += 16;
                  ...
                  ) 
{
  ...
  long orient_for_cur_sens;
  long lenght_for_cur_sens;
  long screen_pos=0;
  long block_pos; 
  ...
  long local_lenght_count;
  long end_local_lenght_count;
...
//                         * P_trace->idx_head.num_test_in_block / P_trace->idx_trc[idx_count].real_len;

  lenght_counter = 0;

  ext_sens = malloc ( sizeof(ext_sens[0]) * (lenght + P_trace->max_sens_shift) );

  fseek( P_trace->ext_idx_file_data, start  * sizeof(ext_sens[0]), SEEK_SET );
  fread(ext_sens, 1, sizeof(ext_sens[0]) * (lenght + P_trace->max_sens_shift), P_trace->ext_idx_file_data);


  while (lenght_counter < (lenght + P_trace->max_sens_shift)) {
...
#pragma warning(default:4127)
      ...
      local_lenght_count = real_len_first_test_in_block;
      end_local_lenght_count = P_trace->idx_trc[idx_count].real_len;

      for ( ; local_lenght_count < end_local_lenght_count; local_lenght_count++)
      {
          test_counter = (local_lenght_count * P_trace->idx_head.num_test_in_block) / P_trace->idx_trc[idx_count].real_len;

          if (lenght_counter < (lenght + P_trace->max_sens_shift))
          {
              prof_sens_num = 0;
              if ( group == 2) {
                  prof_sens_num = FIRST_SENSLINE_SIZE;
              };
              for (sens_counter = beg_sens; sens_counter < end_sens; sens_counter++)
              {
                  if (P_trace->Delete_sens_data[sens_counter] != SENSE_DELETE_FLAG)
                  {
                      ...
                      lenght_for_cur_sens = lenght_counter - P_trace->sens_shift[sens_counter];
...
#pragma warning(default:4127)
                      ...
                      if (P_trace->Orientation_OFF == 0)
                      {
                           ...
                           if (lenght_for_cur_sens >= 0)
                           {
                                if ( group == 0)
                                {
                                   orient_for_cur_sens += ext_sens [lenght_for_cur_sens].orient;
                                } else {
                                   orient_for_cur_sens += 
                                             ext_sens [lenght_for_cur_sens].orient *
                                             P_trace->linesense_size[group] /
                                             P_trace->linesense_size[0];
                                }

                                if ( group == 1)
                                {
                                   orient_for_cur_sens += P_trace->Orientation_shift_2;
                                }
                                if ( group == 2)
                                {
                                   orient_for_cur_sens += P_trace->Orientation_shift_3;
                                }
                                if ( group == 3)
                                {
                                   orient_for_cur_sens += P_trace->Orientation_shift_4;
                                }
                           };
                      }; // if (P_trace->Orientation_OFF == 0)
                      ...
                      if ((lenght_for_cur_sens >= 0) && (lenght_for_cur_sens < lenght))
                      {
                          screen_pos = lenght_for_cur_sens + lenght * orient_for_cur_sens;

                          if ((screen_pos >= lenght*P_trace->real_number_sensor_in_group[group]) ||
                              (screen_pos < 0))
                          {
                             prof_sens_num ++;
                             continue;
...
#pragma warning(default:4127)
                                 {
                                      screen[screen_pos] = (KRTDATA)
                                          Calculate_profil_mm( uarc_data [screen_pos],
                                                               prof_sens_num, P_trace);
                                 } else {
                  ...
                  {
                     odometer_for_cur_dim = (long) drive_wheel_data[start + lenght_counter];// + P_trace->sens_shift[sens_counter]];
                  }
                  ...
                  if ((P_trace->Odometers_tools_use > 0) && (group == 0))
                  {
                     ...
                     {
                         screen[screen_pos] = 115;
                     }

                     if ((sens_counter == P_trace->Odometer_1_sens) && (odometer_for_cur_dim == 1))
                     {
                         screen[screen_pos] = 152;
                     }
                  }
              }; // for sens_counter

              if ((lenght_counter >= 0) && (lenght_counter < lenght))
              {
                 memcpy (&koord_data [lenght_counter], &block_head, sizeof(block_head));
              };
          }; // if (lenght_counter < lenght)
...
//              }
           ...
           case (3): { // acselerometr1
                ...
                if (acselerometr1_data != NULL)
                 {
                    node->Val=(long) acselerometr1_data[my_start];
                 }
           ...
           case (4): { // acselerometr2
                ...
                if (acselerometr2_data != NULL)
                 {
                    node->Val=(long) acselerometr2_data[my_start];
                 }
...
#pragma warning(default:4127)
           ...
           for (local_lenght_count = 0; local_lenght_count < d_trace; local_lenght_count++) {

              if (d_time > 0)
                  rec_ext_sens.time = idx_head.trace_time + (long)(((float)d_time*(float)local_lenght_count)/(float)d_trace);
              else
                  rec_ext_sens.time = idx_head.trace_time + 50 * local_lenght_count;
...
//              rec_ext_sens.termo = (long) gipotenuza;
              ...
              test_counter = (local_lenght_count * file_head.num_test_in_block) / d_trace;
...
//              rec_ext_sens.Thick = block_head.accel_1;
              // Вычислили толщину
